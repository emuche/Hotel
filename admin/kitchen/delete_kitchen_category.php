...
if(isset($_POST['id'])){
	...
	$deleted_cat_name = cat_by_id('kitchen_categories' ,$id);

	delete_cat('kitchen_categories',$id);	

	header('location: delete_kitchen_category.php?category_name='.$deleted_cat_name);
...
<title>Hotel | All kitchen Category</title>
...
<h3 class="text-center text-info"><?php echo strtoupper($user_data['department']) .' DEPARTMENT <br><br> '. ucwords($user_data['last_name'].' '.$user_data['first_name']); ?>'s All kitchen Category Page</h3>
...
<?php foreach(get_categories('kitchen_categories') as $store_data){ ?>

			<tr>
				<td><?php echo $store_data['category']; ?></td>
				<td>
					<form action="all_kitchen_products.php" method="get">
