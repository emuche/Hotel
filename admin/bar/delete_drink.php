...
if (isset($_POST['id']) && !empty($_POST['id'])) {
	$id = $_POST['id'];
	delete_cat('bar_products',$id);
	header('Location: delete_drink.php?success');
...
?>
<title> Hotel | Delete Drinks</title>
<?php
...
<h3 class="text-center text-info"><?php echo strtoupper($user_data['department']) .' DEPARTMENT <br><br> '. ucwords($user_data['last_name'].' '.$user_data['first_name']); ?>'s Delete Drinks Page</h3>
	
<?php   
if(isset($_GET['success']) && empty($_GET['success'])){
	alert_success('Drink Has Been Successfully Deleted');
...
<?php foreach(get_available_store(null, 'bar_products') as $store_data){ ?>
