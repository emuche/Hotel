...
if (isset($_POST['id']) && !empty($_POST['id'])) {
	$id = $_POST['id'];
	delete_cat('rooms_available',$id);
	header('Location: delete_room_number.php?success');
...
?>
<title> Hotel | Delete Room Number</title>
<?php
...
<h3 class="text-center text-info"><?php echo strtoupper($user_data['department']) .' DEPARTMENT <br><br> '. ucwords($user_data['last_name'].' '.$user_data['first_name']); ?>'s Delete Room Number Page</h3>
	
<?php   
if(isset($_GET['success']) && empty($_GET['success'])){
	alert_success('Room Has Been Successfully Deleted');
...
<?php foreach(get_available_store(null, 'rooms_available') as $store_data){ ?>
