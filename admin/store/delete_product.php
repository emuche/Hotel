...
if (isset($_POST['id']) && !empty($_POST['id'])) {
	$id = $_POST['id'];
	delete_cat('store_products',$id);
	header('Location: delete_product.php?success');
...
?>
<title> Hotel | Delete Products</title>
<?php
...
<h3 class="text-center text-info"><?php echo strtoupper($user_data['department']) .' DEPARTMENT <br><br> '. ucwords($user_data['last_name'].' '.$user_data['first_name']); ?>'s Delete Product Page</h3>
	
<?php   
if(isset($_GET['success']) && empty($_GET['success'])){
	alert_success('Product Has Been Successfully Deleted.');
...
<?php foreach(get_available_store(null, 'store_products') as $store_data){ ?>
